...
const BudgetConfiguration: React.FC<props> = ({ onSectionsChange}) => {
  ...
  useEffect(() => {
    ...
    // Recalculate allocated amounts for each section based on latest salary/budget
    const updatedSections = sectionArr.map((section) => ({
      ...section,
      allocatedAmount: ((salary * budgetPercentage) / 100) * (section.percentage / 100),
    }));

    onSectionsChange(updatedSections);
  }, [sectionArr, salary, budgetPercentage, onSectionsChange]);
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_BASE}/fetchBudgetConfig`, {
        params: {
          profileId: profileId,
          month: month,
          year: year,
  ...
  const totalPercentage = sectionArr.reduce(
  ...
  try {
    const res = await axios.get(`${API_BASE}/value`, {
      params: {
        profileId,
        configName: "SECTION_TYPES_DEBIT",
        month,
        year,
      },
    });
    ...
    if (res.data) {
      ...
      setSectionOptions(values);
    } else {
      setSectionOptions([]);
    }
  } catch (err) {
    console.error("Failed to load SECTION_OPTIONS config:", err);
    setSectionOptions([]);
  }
};
  ...
  const handleAddOrUpdate = () => {
    if (
      !selectedSectionName ||
      !selectedSectionPercentage ||
      selectedSectionPercentage < 0
    )
      return;
  ...
  const saveBudgetConfig = () => {
    if (!profileId || !salary || !month || !year) {
      const errorMsg = [];
      if (!profileId) {
    ...
    axios.post(`${API_BASE}/saveBudgetConfig`, budgetConfig).then((res) => {
    ...
    <div>
      ...
      <div className="d-flex flex-column flex-sm-row align-items-center gap-2 mb-3">
        ...
        <input
          ...
          value={selectedSectionPercentage}
          onChange={(e) => setSelectedSectionPercentage(Number(e.target.value))}
        />
